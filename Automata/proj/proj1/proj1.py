...
def GetInput(fn):
    inputStringLis = []
    inputFile = open(fn)
    REString = inputFile.readline().strip()
...
def REMatch():
    for i in xrange(11):
        inputFn = "D:\\\Dropbox\\project\\C++\\homework\\automata\\project1\\resources\\q%s.txt" % (i + 1)
        outputFn = "D:\\\Dropbox\\project\\C++\\homework\\automata\\project1\\resources\\a%s.txt" % (i + 1)
        REString, inputStringLis = GetInput(inputFn)
        print REString
        curNFA = RE2NFA(REString)
