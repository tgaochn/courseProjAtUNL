...
def func():
    ...
    userIdLis2 = userRltDic.keys()

    targetUserIdSet = set(userIdLis1).intersection(set(userIdLis2))
    targetUserRltDic = [(userId, msgCnt) for userId, msgCnt in userRltDic.iteritems() if userId in targetUserIdSet]
    finalUserId = max(targetUserRltDic, key=lambda x:x[1])

    elapsed3 = time.clock() - start

    print targetUserRltDic
    print finalUserId
    print("Time used:", elapsed1 + elapsed2 + elapsed3)
# end_func
