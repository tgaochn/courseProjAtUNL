...
def do_eval(sess,
  ...
  loss_value = sess.run(loss2,feed_dict=feed_dict)

  print('  Num examples: %d  Num correct: %d  Precision @ 1: %0.04f  Loss=%0.04f' %
        (num_examples, true_count, precision,loss_value))
def conv2d(x, W):
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    loss2 = loss(logits, labels_placeholder)

    # Add to the Graph the Ops that calculate and apply gradients.
    train_op = training(loss2, FLAGS.learning_rate)
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      _, loss_value = sess.run([train_op, loss2],
                               feed_dict=feed_dict)
